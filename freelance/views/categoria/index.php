...
$this->registerJs(<<<JS
let timeout;
$('#categoria-search-input').on('input', function() {
    clearTimeout(timeout);
    timeout = setTimeout(function() {
        $.pjax.submit($('#auto-search-form'), '#categoria-pjax');
    }, 500);
});
JS);
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                $.pjax.reload({container: '#categoria-pjax', async: false}); // async: false para esperar a que pjax termine
            } else {
...
<div class="page-content">
    ...
    <div class="col d-flex justify-content-between align-items-start">
        <h6 class="mb-0 text-uppercase">Categorías <dl><?= $dataProvider->getTotalCount() ?></dl>
        </h6>
        <div>
            <?= Html::a('<i class="bx bx-plus mr-1"></i> Crear Categoría', ['create', 'view' => 'modal'], [
                ...
                <div class="row">
                    <div class="col-sm-12 col-md-6">
                        <div class="dt-buttons btn-group">
                            <?= Html::a('Excel', ['categoria/export-excel'], [
                                'target' => '_blank',
                                'class' => 'btn btn-light buttons-excel buttons-html5',
                                'data-pjax' => 0,
                            ]) ?>
                            <?= Html::a('PDF', ['categoria/export-pdf'], [
                                'target' => '_blank',
                                'class' => 'btn btn-light buttons-excel buttons-html5',
                                'data-pjax' => 0,
                            ]) ?>
                            <?= Html::a('Print', ['categoria/print'], [
                                'target' => '_blank',
                                'class' => 'btn btn-light buttons-excel buttons-html5',
                                'data-pjax' => 0,
                            ]) ?>
                            ...
                            [
                                ...
                                'buttons' => [
                                    ...
                                    'delete' => fn($url, $model) => Html::a('<i class="bx bx-trash"></i>', $url, [
                                        ...
                                        'class' => 'btn btn-light',
                                        'data-confirm' => '¿Está seguro de que desea eliminar la categoría: "' . $model->cat_nombre . '"?',
                                        'data-method' => 'post',
                                        'data-pjax' => '1',
                                    ]),
                                ],
