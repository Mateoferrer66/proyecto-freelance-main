...
$this->registerJs(<<<JS
let timeout;
$('#provincia-search-input').on('input', function() {
    clearTimeout(timeout);
    timeout = setTimeout(function() {
        $.pjax.submit($('#auto-search-form'), '#provincias-pjax');
    }, 500);
});
JS);
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                $.pjax.reload({container: '#provincias-pjax', async: false}); // async: false para esperar a que pjax termine
            } else {
...
<div class="page-content">
    ...
    <div class="col d-flex justify-content-between align-items-start">
        ...
        <div>
            <?= Html::a('<i class="bx bx-plus mr-1"></i> Crear Provincia', ['create', 'view' => 'modal'], [
                            ...
                            [
                                ...
                                'buttons' => [
                                    'view' => fn($url, $model) => Html::a('<i class="bx bx-show"></i>', $url, [
                                        'title' => 'Ver Provincia: ' . $model->prv_nombre,
                                        'class' => 'btn btn-light',
                                        'data-bs-toggle' => 'modal',
                                        'data-bs-target' => '#action-modal'
                                    ]),
                                    'update' => fn($url, $model) => Html::a('<i class="bx bx-edit"></i>', $url, [
                                        'title' => 'Editar Provincia: ' . $model->prv_nombre,
                                        'class' => 'btn btn-light',
                                        'data-bs-toggle' => 'modal',
                                        'data-bs-target' => '#action-modal'
                                    ]),
                                    'delete' => fn($url, $model) => Html::a('<i class="bx bx-trash"></i>', $url, [
                                        ...
                                        'class' => 'btn btn-light',
                                        'data-confirm' => '¿Está seguro de que desea eliminar la provincia: "' . $model->prv_nombre . '"?',
                                        'data-method' => 'post',
                                        'data-pjax' => '1',
                                    ]),
                                ],
                                'urlCreator' => fn($action, Provincia $model, $key, $index, $column) =>
                                Url::toRoute([$action, 'prv_id' => $model->prv_id, 'view' => ($action === 'delete' ? null : 'modal')]),
                            ],
                        ],
                    ]); ?>
                </div>
