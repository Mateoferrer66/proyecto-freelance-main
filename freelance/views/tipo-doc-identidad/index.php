...
$this->registerJs(<<<JS
let timeout;
$('#tipo-doc-identidad-search-input').on('input', function() {
    clearTimeout(timeout);
    timeout = setTimeout(function() {
        $.pjax.submit($('#auto-search-form'), '#tipo-doc-identidad-pjax');
    }, 500);
});
JS);
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                $.pjax.reload({container: '#tipo-doc-identidad-pjax', async: false}); // async: false para esperar a que pjax termine
            } else {
...
<div class="page-content">
    ...
    <div class="col d-flex justify-content-between align-items-start">
        <h6 class="mb-0 text-uppercase">Tipos de Documento de Identidad <dl><?= $dataProvider->getTotalCount() ?></dl>
        </h6>
        <div>
            <?= Html::a('<i class="bx bx-plus mr-1"></i> Crear Tipo Doc Identidad', ['create', 'view' => 'modal'], [
                'class' => 'btn btn-success radius-30',
                'title' => 'Crear Tipo Doc Identidad',
    ...
    <hr />
    <div class="row">
        <?php Pjax::begin(['id' => 'tipo-doc-identidad-pjax']); ?>
                    ...
                    <div class="col-sm-12 col-md-6">
                        <div class="dataTables_filter">
                            ...
                            <?= $form->field($searchModel, 'tdo_nombre', ['template' => '{input}'])
                                ->textInput([
                                    'placeholder' => 'Buscar por nombre de tipo de documento...',
                                    'class' => 'form-control form-control-sm',
                                    'id' => 'tipo-doc-identidad-search-input',
                                    'autocomplete' => 'off'
                                ]) ?>
                            <?php ActiveForm::end(); ?>
                        </div>
                            ...
                            [
                                ...
                                'buttons' => [
                                    'view' => fn($url, $model) => Html::a('<i class="bx bx-show"></i>', $url, [
                                        'title' => 'Ver Tipo de Documento: ' . $model->tdo_nombre,
                                        'class' => 'btn btn-light',
                                        'data-bs-toggle' => 'modal',
                                        'data-bs-target' => '#action-modal'
                                    ]),
                                    'update' => fn($url, $model) => Html::a('<i class="bx bx-edit"></i>', $url, [
                                        'title' => 'Editar Tipo de Documento: ' . $model->tdo_nombre,
                                        'class' => 'btn btn-light',
                                        'data-bs-toggle' => 'modal',
                                        'data-bs-target' => '#action-modal'
                                    ]),
                                    'delete' => fn($url, $model) => Html::a('<i class="bx bx-trash"></i>', $url, [
                                        ...
                                        'class' => 'btn btn-light',
                                        'data-confirm' => '¿Está seguro de que desea eliminar el tipo de documento: "' . $model->tdo_nombre . '"?',
                                        'data-method' => 'post',
                                        'data-pjax' => '1',
                                    ]),
                                ],
                                'urlCreator' => fn($action, TipoDocIdentidad $model, $key, $index, $column) =>
                                Url::toRoute([$action, 'id' => $model->tdo_id, 'view' => ($action === 'delete' ? null : 'modal')]),
                            ],
                        ],
                    ]); ?>
                </div>
